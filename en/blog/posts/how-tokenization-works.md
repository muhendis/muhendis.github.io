...
- On this blog: [Inside the embedding layer](post.html?slug=inside-the-embedding-layer) — discrete tokens to continuous geometry —, [How embeddings work](post.html?slug=how-embeddings-work) — what happens to the integer after the lookup —, [How LLMs actually work](post.html?slug=how-llms-work) — the layers that consume those vectors —, and [Optimizing LLM cost and latency](post.html?slug=optimizing-llm-cost-and-latency) — why token counts are the unit your bill is written in.
